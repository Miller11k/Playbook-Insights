...
export async function getSpecialTeamsStats(req: Request, res: Response) {
    ...
    try {
        ...
        const filteredResult = filterNullValues(result.rows, "special_teams");
        

        if ((filteredResult.length === 0) && (result.rows.length != 0)) {
            res.status(204).json({ error: "All special team stats found for specified criteria were null." });
            return;
        ...
        res.status(200).json(filteredResult.map(row => row.special_teams));
    } catch (error) {
